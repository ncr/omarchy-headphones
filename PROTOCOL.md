...
`06 01` answers with the six bytes and nothing around them, on every model:
...
connect, needs no model table, and cannot drift as firmware moves fields about.
...
So the query goes out unconditionally after the state packet, and its reply
stands over whatever the offset produced. A device that never answers it keeps
precisely the behaviour it has today.
